# This code demonstrates the method used to calculate Suitability Scores for each pair of markers.
# For each School Serial Number, a pair of markers is selected at random from those with the highest Suitability Score, S.
# The Suitability Score is calculated as: S = r - N * n,
# where:
#   r = a measure of how suitable the pair is, based on how soon after delivery of a test the two markers will next be available.
#       (A linear transformation has been applied so that higher r values indicate higher suitability.)
#   N = a user-defined weight to control how much to penalise repeated use of the same pair across different schools. It is currently set at 10. 
#   n = the number of times this particular pair of markers has already been selected or used since the assignment process began.
# This code also outputs a spreadsheet called 'Pre_Allocation_of_Markers.xlsx', showing the final allocation of Markers 1 and 2 to each school.
...
# Construct the full path to the marker data file
markers_file_path <- file.path(working_dir, "Marker_Data.xlsx")
...
# Construct the full path to the marker data file
schedule_file_path <- file.path(working_dir, "School_Test_Delivery_Schedule.xlsx")
...
# Initialise a global usage tracker (marker pair -> count)
...
update_global_usage <- function(pair_name) {
  # Initialise count if not exists
...
# Set column widths
setColWidths(wb, sheet = 1, cols = 1:5, widths = c(15, 15, 15, 15, 15))

# Create a bold style for headers
header_style <- createStyle(textDecoration = "bold")

# Apply bold style to the header row
addStyle(wb, sheet = 1, style = header_style, rows = 1, cols = 1:5)

# Freeze the header row
freezePane(wb, sheet = 1, firstRow = TRUE)

# Save the workbook with proper column widths
output_file_path <- file.path(working_dir, "Pre_Allocation_of_Markers.xlsx")
